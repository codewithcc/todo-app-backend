...
async def create_todo(todo: Todo):
    try:
        ...
        raw_data = {k: v for k, v in todo.dict().items() if v is not None}
        ...
        return {"success": True, "data": todo_helper(new_todo)}
...
async def update_todo(todo_id: str, updated_todo: Todo):
    try:
        ...
        update_data = {
            k: v for k, v in updated_todo.dict(exclude_unset=True).items()
            if v is not None
        }
